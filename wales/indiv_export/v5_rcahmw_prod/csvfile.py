...
class CsvWriter(Writer):
    ...
    def write_resources(self, graph_id=None, resourceinstanceids=None, **kwargs):
        ...
        with open(csv_name, 'w') as f:
            newwriter = csv.DictWriter(f, delimiter=",", fieldnames=csv_header)
            newwriter.writeheader() # Headings from mapping file
            ...
            for resourceinstanceid, tiles in self.resourceinstances.items():
                ...
                for tile in tiles:
                    ...
                    if other_group_record != {"ResourceID": resourceinstanceid}:
                        continue
                        # Don't want a huge list of other group records
                        #other_group_records.append(other_group_record)

                # This is the process of writing each resource to the CSV
                if csv_record != {"ResourceID": resourceinstanceid}:
                        if "populated_node_groups" in csv_record:
                            del csv_record["populated_node_groups"]

                        newwriter.writerow({k: str(v) for k, v in list(csv_record.items())}) 
                        
                        # csv_records contains each record
                        #csv_records.append(csv_record)
                        # Comment it out to prevent huge list creation
            ...
            if self.single_file is not True:
                ...
                for csv_record in csv_records:
                    #if "populated_node_groups" in csv_record:
                    #    del csv_record["populated_node_groups"]
                    ## csv_records should be empty but write rows if not
    ...
    def write_resource_relations(self, file_name):
        ...
        with open(csv_name, 'w') as f:
            if self.graph_id != settings.SYSTEM_SETTINGS_RESOURCE_MODEL_ID:
                ...
                # Add our new CSV writer command
                newwriter = csv.DictWriter(f, delimiter=",", fieldnames=csv_header)
                newwriter.writeheader() # Headings from mapping file
                ...
                for relation in relations:
                    ...
                    newwriter.writerow({k: str(v) for k, v in list(relation.items())})
